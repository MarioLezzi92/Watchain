...
// Aggiunta prop 'variant' (default "market")
export default function WatchCard({ item, onOpen, variant = "market" }) {
  ...
  // È listato se ha un prezzo (anche 0 per i casi di errore)
  const isListed = item?.priceLux != null;

  // LOGICA BANNER: Mostra SOLO se siamo in Inventario E l'item è listato
    ...
    >
      ...
      <div className="w-full aspect-square bg-zinc-900 relative overflow-hidden">
        <img
          src={imgSrc}
          alt="watch"
          // Se c'è il banner, sbiadiamo leggermente l'immagine
        ...
        {/* BANNER: Appare SOLO in Inventario */}
        {showBanner && (
      ...
      <div className="p-5 flex flex-col gap-1 w-full border-t border-white/10">
        ...
        {/* Prezzo: Nel market lo mostriamo sempre. In inventario anche. */}
        <div className="mt-4 flex items-center justify-end h-6">
           {isListed ? (
             <span className="text-[#D4AF37] font-bold text-xl">{String(item.priceLux)} <span className="text-xs text-red-100/50 font-normal">LUX</span></span>
           ) : (
